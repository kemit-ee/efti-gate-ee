...
3. **Archival by CronManager.** Non-latest rows moved by external Quartz scheduler — and it all runs through **Ruuter + ReSql**, never a DB function and never a foreign-data wrapper. Cold storage for `consignments` is a **separate PostgreSQL instance** (`archive-database` / DB `efti_archive`, table in `DSL/Liquibase/archive-init.sql` — plain types, no CHECK/FK, `row_id` PK). ReSql has a second datasource `archive`; SQL files under `DSL/Resql/archive/` run against it (`resql.yaml` `project_datasource_map`). CronManager calls `POST /ops/v1/archive-consignments` (`Authorization: Bearer ARCHIVE_OPS_TOKEN`, guard `DSL/Ruuter/ops/.guard.yml`; one batch per call, re-invoke until `candidates` is 0). The route carries rows between the two datasources: `efti/select_archivable_consignments.sql` (superseded rows whose newer sibling is older than `olderThanDays`, default 2, so the current row incl. a `DELETED` tombstone never moves) → `archive/insert_archived_consignments.sql` (idempotent, `ON CONFLICT (row_id) DO NOTHING`) → `archive/verify_archived_consignments.sql` (read back from the archive DB) → **only on 100 % confirmation** → `efti/delete_consignments.sql`, a **real DELETE** guarded by `EXISTS (newer sibling)` so a current row can never be removed. A mismatch logs `FAILED` and deletes nothing. `POST /ops/v1/purge-archive` runs `archive/purge_archived_consignments.sql` (cold-storage retention, `keepDays` default 2555). Full history (live + archived): `GET /admin/v1/consignment-history?datasetId=…`.
