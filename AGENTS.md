...
- **Input contract** (`declaration:` + `validate_input`, checked by `scripts/validate-dsl.py`; Ruuter's own `dsl-lint` runs alongside it). Every route in the six projects declares its inputs and enforces the required ones. On `ruuter:0.10.0-rc` (issue turnerrainer/Ruuter#75 landed in 0.9.12-rc):
...
- Runtime: `turnerrainer/resql:0.4.2-alpha`, distroless UID 65532. Use `/app/resql health --url http://127.0.0.1:8090/health`; no shell/curl is available. Config/SQL files must be readable by UID 65532.
...
- TIM uses `0.4.0-alpha` with required introspection client auth. Provision `TIM_INTROSPECT_RUUTER_SECRET` outside local development. Existing `/jwt/userinfo` calls use the compatibility gate's default-off posture.
...
- `.github/workflows/e2e.yml` — GitHub Actions:
  - `dsl-validate` — runs Ruuter's `dsl-lint` + `dsl-test` straight from the runtime image
    (`turnerrainer/ruuter:0.10.0-rc`, which ships both binaries since #83) on both DSL roots +
