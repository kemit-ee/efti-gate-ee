...
- **Input contract** (`declaration:` + `validate_input`, checked by `scripts/validate-dsl.py`; Ruuter's own `dsl-lint` runs alongside it). Every route in the six projects declares its inputs and enforces the required ones. On `ruuter:0.10.1-rc` (issue turnerrainer/Ruuter#75 landed in 0.9.12-rc):
...
- CronManager (the scheduled-job caller for `DSL/Ruuter/ops/**`, e.g. the archive sweep) is [turnerrainer/cronmanager](https://github.com/turnerrainer/cronmanager), latest `v0.2.2-alpha` — corrected from a stale `Buerostack/CronManager` reference across the docs. It's not built/deployed from this repo (no Dockerfile here; operators drop `docs/specs/deploy/cronmanager-*.yaml` into its `dsl_path` externally), but its DSL schema is load-bearing on ours: its HTTP job type is bare `method`+`url` only — **no `headers:`, no `body:`, no runtime env-var interpolation of job fields** (`serde(deny_unknown_fields)` hard-fails the whole job file on an unrecognised key). That's why `DSL/Ruuter/ops/.guard.yml` checks a `?opsToken=` **query param**, not an `Authorization` header, and why `archive-consignments.yml`/`purge-archive.yml` read `incoming.params.*` instead of `incoming.body.*`. The `${GATE_BASE_URL}`/`${ARCHIVE_OPS_TOKEN}` placeholders in the job YAML files are template markers for a deploy-time rendering step, not something CronManager itself substitutes.
...
- `DSL-tests/**/*.test.yml` — Ruuter `dsl-test` scenarios (`mode: inprocess` — HTTP through the
  ...
  Run via `docker run --rm -v "$PWD:/workdir" -w /workdir turnerrainer/ruuter:0.10.1-rc dsl-test --dsl DSL/Ruuter --tests DSL-tests --constants constants.ini` (the binaries ship in the runtime image since 0.9.14-rc).
...
- `.github/workflows/e2e.yml` — GitHub Actions:
  - `dsl-validate` — runs Ruuter's `dsl-lint` + `dsl-test` straight from the runtime image
    (`turnerrainer/ruuter:0.10.1-rc`, which ships both binaries since #83) on both DSL roots +
