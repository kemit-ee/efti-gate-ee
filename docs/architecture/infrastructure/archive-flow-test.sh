#!/bin/sh
# End-to-end check of the consignments archive flow (SELECT live -> INSERT archive DB -> verify
# against archive DB -> real DELETE from live), plus purge and the history read. Run against a dev
# stack. The e2e http-tests only cover the guard + the "nothing to archive" path because they
...
# including D2's DELETED tombstone — stays in public.consignments; the moved rows are gone from
# the live DB and present in the separate archive DB.
...
INSERT INTO public.consignments (dataset_id, platform_id, gate_id, xml, status, main_transport_id, created_at) VALUES
 ...
 ('d1111111-1111-1111-1111-111111111111','p1','EU-EE','<x>V3</x>','ACTIVE','V3-current', now()-interval '1 hour'),
...
echo "--- separate archive DB (expect V1, W1) ---"
