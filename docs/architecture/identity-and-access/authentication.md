...
flowchart TD
    ...
    Channel -- Platform API --> MTLSp[X-Api-Key header<br/>resolved against active platforms<br/>by SHA-256 of the key]
...
## 3. Platform API-key pipeline

Platforms authenticate to the **m2m Ruuter** (port 8087, [ADR-005](../decisions/005-m2m-ruuter-split.md)) with an API key ([ADR-004](../decisions/004-platform-api-key.md)).

- Caller sends `X-Api-Key: <key>` on every Platform API request.
- The `m2m/POST/platform/.guard` hashes the key (SHA-256) and resolves it against the latest non-deleted `platforms` row (`platforms.api_key_hash`).
- 0 rows → `401 unauthorized`. >1 rows → `403 forbidden-multi-platform` (operator misconfiguration).
- The key is generated by an admin (`POST /api/v1/platforms/{id}/api-key`), returned once, and stored only as the hash plus an 8-hex-char hint. Log redaction strips `X-Api-Key`.
