...
The gate runs two Ruuter instances ([ADR-005](../decisions/005-m2m-ruuter-split.md)): `efti` for the admin UI API (TARA JWT), and `m2m` for the machine surface (Platform API, Authority API, peer-gate eDelivery, X-Road). Each `m2m` subdirectory has its own guard. The **Platform API** is authenticated by an `X-Api-Key` header resolved against `platforms.api_key_hash` ([ADR-004](../decisions/004-platform-api-key.md)) — the key is generated by an admin, shown once, and stored only as a SHA-256 hash. **Peer-gate eDelivery** routes are reached only from the local `edelivery` service, which has already terminated AS4 / WS-Security and the EU-Trust-Service mTLS at the access point; the `m2m` Ruuter is not exposed outside the network perimeter. Operators are responsible for ensuring that perimeter is not bypassable. STRIDE per-surface threat model documentation is a deployment-time deliverable (see `docs/cfr/security-and-compliance/security.md` AC).
