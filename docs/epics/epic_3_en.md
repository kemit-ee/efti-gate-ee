...
- [ ] Re-sending same `datasetId` with new data → INSERT a new `consignments` row sharing the same `dataset_id` (status='active'); the previous row remains in the table but is no longer the latest. Authority latest-row reads return the new row → `200 OK`
