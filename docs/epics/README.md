# eFTI Gate — Epics Index

> Auto-split from `../efti_full_epics_en.md`. See also [full file](../efti_full_epics_en.md).


## [THEME 1 — Identity and Access](theme_1_en.md)

- [EPIC 1 — User Management and RBAC](epic_1_en.md)
- [EPIC 2 — Authentication](epic_2_en.md)
- [EPIC 23 — Authentication and Access Flows](epic_23_en.md)

## [THEME 2 — Core Functionality](theme_2_en.md)

- [EPIC 3 — Identifier Management (Platform API)](epic_3_en.md)
- [EPIC 4 — Identifier Search (Authority API)](epic_4_en.md)
- [EPIC 5 — Dataset Retrieval and Follow-up](epic_5_en.md)
- [EPIC 24 — Identifier Search and Dataset Retrieval Flows](epic_24_en.md)

## [THEME 3 — Registry Management](theme_3_en.md)

- [EPIC 6 — Gate Registry Management (Admin API)](epic_6_en.md)
- [EPIC 7 — Platform Registry Management (Admin API)](epic_7_en.md)
- [EPIC 8 — Authority Registry Management (Admin API)](epic_8_en.md)
- [EPIC 9 — Consignment Management (Admin API)](epic_9_en.md)

## [THEME 4 — Integrations](theme_4_en.md)

- [EPIC 10 — eDelivery AS4 Integration](epic_10_en.md)
- [EPIC 11 — X-Road Integration (EE extension)](epic_11_en.md)
- [EPIC 25 — eDelivery AS4 Message Flow](epic_25_en.md)

## [THEME 5 — Infrastructure](theme_5_en.md)

- [EPIC 12 — Scalability and Statelessness](epic_12_en.md)
- [EPIC 13 — Health Checks and Graceful Shutdown](epic_13_en.md)
- [EPIC 26 — Append-Only Archival via CronManager](epic_26_en.md)

## [THEME 6 — Security and Compliance](theme_6_en.md)

- [EPIC 14 — Security](epic_14_en.md)
- [EPIC 15 — Audit and GDPR Compliance](epic_15_en.md)

## [THEME 7 — Observability](theme_7_en.md)

- [EPIC 16 — Logging and Observability](epic_16_en.md)
- [EPIC 17 — Monitoring and Alerting](epic_17_en.md)

## [THEME 8 — Software Quality](theme_8_en.md)

- [EPIC 18 — Test Coverage and Quality](epic_18_en.md)
- [EPIC 19 — API Standardisation](epic_19_en.md)
- [EPIC 20 — CI/CD and Supply Chain Security](epic_20_en.md)

## [THEME 9 — User Interfaces](theme_9_en.md)

- [EPIC 21 — Authority UI (AAP — H2M Interface)](epic_21_en.md)
- [EPIC 22 — Admin UI](epic_22_en.md)
