...
- **[CronManager](https://github.com/turnerrainer/cronmanager)** is a strict requirement, not optional. Deployed as a sibling container/Pod alongside the gate. CronManager owns every scheduled task — including the **append-only archival sweep** (Epic 26) that moves non-latest rows of every operational table to archival storage on a configurable cron schedule. The gate's runtime never schedules its own jobs; it only exposes the `ops/` endpoints that CronManager calls. See `docs/specs/deploy/cronmanager-archive.yaml` for the canonical job definition — note CronManager's HTTP job type has no headers/body support, so the ops token travels as a `?opsToken=` query param.
