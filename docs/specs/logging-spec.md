...
| `efti.required` | string | Flag the route required when `user.access.denied` fires | `"authenticated"` (Admin API / Authority API). Platform identity is mTLS (`FORBIDDEN_NO_PLATFORM` / `FORBIDDEN_MULTI_PLATFORM`); CronManager `admin/*` denial is plain `FORBIDDEN` without `efti.required`. |
...
```json
{
  ...
  "user": { "id": "04fa30eb-eb08-11f0-b506-3c9c0f2eb459" },
  "efti": { "error.code": "FORBIDDEN", "required": "authenticated", "audit": true },
...
| `user.id` | Omit when authentication failed (no user resolved). |
...
```json
{
  ...
  "user": { "id": "175791a3-da82-11f0-b10c-3c9c0f2eb459" },
...
| `user.access.denied` | `required` (`"authenticated"`), `target_resource` | |
