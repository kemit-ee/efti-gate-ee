...
| `efti.required` | string | Flag the route required when `user.access.denied` fires | `"is_admin"` (Admin API) / `"is_admin"` (Authority API). Platform identity is mTLS (`FORBIDDEN_NO_PLATFORM` / `FORBIDDEN_MULTI_PLATFORM`); CronManager `admin/*` denial is plain `FORBIDDEN` without `efti.required`. |
...
```json
{
  ...
  "user": { "id": "04fa30eb-eb08-11f0-b506-3c9c0f2eb459", "is_admin": true },
...
| `user.id` / `user.is_admin` | Omit when authentication failed (no user resolved). |
...
```json
{
  ...
  "user": { "id": "175791a3-da82-11f0-b10c-3c9c0f2eb459", "is_admin": true },
...
| `user.access.denied` | `required` (`"is_admin"` or `"is_admin"`), `target_resource` | |
