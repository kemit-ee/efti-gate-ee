...
| `ARCHIVE_OPS_TOKEN` | Ruuter (`constants.ini`, `[#ARCHIVE_OPS_TOKEN]`), `DSL/Ruuter/ops/.guard.yml`; välise CronManager'i saadetav `?opsToken=`-query-parameeter | Ainus autentimine `POST /ops/v1/archive-consignments` ja `POST /ops/v1/purge-archive` peale — literal compare, mitte JWT. **Selle valdaja saab käivitada saadetiste külmladustamise/purge'imise mistahes ajal.** Vt `docs/architecture/infrastructure/append_only_archival.md` | `dev-archive-ops-token-change-me` | Sama probleem mis `INTERNAL_SERVICE_TOKEN`-il: `constants.ini` on image'isse `COPY`-tud, `${ENV}`-asendust ei ole — muutmiseks tuleb ruuter image uuesti buildida. `DSL/Ruuter/ops/` **ei ole nginx'i kaudu proksitud** (vt §6) — ainus kaitse on, et CronManager pöördub otse konteinerivõrgus, mitte ingressi kaudu. Query-param, mitte Bearer-päis, kuna [turnerrainer/cronmanager](https://github.com/turnerrainer/cronmanager)'i HTTP-töö tüübil pole `headers:`/`body:` tuge — vt `docs/specs/deploy/cronmanager-archive.yaml` |
