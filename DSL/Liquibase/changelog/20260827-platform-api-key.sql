...
-- Platforms authenticate to the m2m Ruuter with an API key sent in the X-Api-Key
-- header (ADR-004, 2026-08-25 — Rainer Türner, Sten Viljus, Anton Keks).
-- The key is never stored in clear: only its SHA-256 hash is kept. `api_key_hint`
-- is the first 8 hex chars of that hash, shown in the admin UI so an operator can
...
-- Give the seeded mock platform a known key so the local http tests can call the
-- platform-facing m2m routes. Matches the value the mock-platform DSL checks.
UPDATE platforms
