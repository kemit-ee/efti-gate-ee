...
COMMENT ON TABLE  users IS 'Human users of the gate. Append-only: every role/subset change INSERTs a new row with the same id. Primary auth is TARA OIDC; gate matches JWT `sub` against `tara_sub`.';
...
COMMENT ON COLUMN users.email             IS 'Display/contact email. CITEXT for case-insensitive match. NOT the auth identifier; tara_sub is.';
...
COMMENT ON COLUMN users.roles             IS 'Role → scope-IDs mapping: {"AUTHORITY":["auth-mta"]} or {"ADMIN":["eu-xx01"]}.';
COMMENT ON COLUMN users.subsets           IS 'eFTI subsets this user is permitted to request. Must be a subset of the authority''s subsets.';
...
CREATE INDEX IF NOT EXISTS idx_users_email_latest ON users (email, created_at DESC);
