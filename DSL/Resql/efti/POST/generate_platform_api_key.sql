/*
description: generate a new X-Api-Key for a platform (ADR-004). Appends a new
  platforms row carrying every field forward from the latest non-deleted row and
  replacing the api_key_* columns. Returns the plaintext key EXACTLY ONCE — it is
  stored only as a SHA-256 hash and cannot be retrieved again.
params:
